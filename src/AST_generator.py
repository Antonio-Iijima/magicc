...
:param str path: Path to a folder containing at least a `syntax.txt` file.
    ...
    TERMINALS = {
        ...
        for pattern in alternatives
        if len(pattern) == 1 and is_terminal(pattern[0])
    }
...
GRAMMAR = {{
    {",\n    ".join(f'''{rule}{" " * (offset - len(rule))} : [{
        ",".join(f"[{", ".join(s[1:-1] if is_nonterminal(s) else f"r'{s}'" for s in alternative)}]"
...
TERMINALS = {{
   {",\n   ".join((f"r'{t}' : {r}" for t, r in TERMINALS.items()))},
   ' ' : lambda *_: None
}}
